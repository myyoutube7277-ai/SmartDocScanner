...
object ScanProcessor {
    ...
    fun autoCrop(src: Bitmap): Bitmap {
        ...
        val w = max(distance(tl, tr), distance(bl, br)).toInt().coerceAtLeast(2)
        val h = max(distance(tl, bl), distance(tr, br)).toInt().coerceAtLeast(2)
    ...
    private fun detectQuad(src: Bitmap): Quad? {
        ...
        try {
            ...
            val lxRange = (w * .03f).toInt()..(w * .47f).toInt()
            val rxRange = (w * .53f).toInt()..(w * .97f).toInt()
            val tyRange = (h * .03f).toInt()..(h * .47f).toInt()
            val byRange = (h * .53f).toInt()..(h * .97f).toInt()
            val (lx, ls) = bestV(lxRange.first, lxRange.last)
            val (rx, rs) = bestV(rxRange.first, rxRange.last)
            val (ty, ts) = bestH(tyRange.first, tyRange.last)
            val (by, bs) = bestH(byRange.first, byRange.last)
            val reliable = ls >= 28f && rs >= 28f && ts >= 28f && bs >= 28f &&
                rx - lx >= w * .30f && by - ty >= h * .30f
            if (!reliable) return null
            return Quad(
                arrayOf(
                    PointF(lx / scale, ty / scale), PointF(rx / scale, ty / scale),
                    PointF(rx / scale, by / scale), PointF(lx / scale, by / scale)
                ),
                rx - lx, by - ty
            )
    ...
    fun rotate(src: Bitmap): Bitmap = Bitmap.createBitmap(
    ...
    fun filter(src: Bitmap, mode: String): Bitmap {
        ...
        val input = IntArray(w * h); val output = IntArray(w * h)
        val lum = IntArray(w * h); val hist = IntArray(256)
        src.getPixels(input, 0, w, 0, 0, w, h)
        for (i in input.indices) {
            val c = input[i]
            val v = (0.299f * Color.red(c) + 0.587f * Color.green(c) + 0.114f * Color.blue(c)).toInt().coerceIn(0, 255)
            lum[i] = v; hist[v]++
        }
        ...
        for (y in 0 until h) for (x in 0 until w) {
            ...
            when (mode) {
                ...
                "High Contrast", "Clean White", "Enhance" -> v = (((v - 128) * 1.28f) + 128).toInt().coerceIn(0, 255)
            }
    ...
    private fun localThreshold(l: IntArray, w: Int, h: Int, x: Int, y: Int, global: Int): Int {
        ...
        var yy = max(0, y - r)
        while (yy <= min(h - 1, y + r)) {
            var xx = max(0, x - r)
            while (xx <= min(w - 1, x + r)) { sum += l[yy * w + xx]; n++; xx += 3 }
            yy += 3
        }
        return ((sum / max(1, n)) * .92f).toInt().coerceIn(global - 45, global + 45)
    }

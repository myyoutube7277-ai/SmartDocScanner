...
object ScanProcessor {
    ...
    fun autoCrop(src: Bitmap): Bitmap {
        val q = detectQuad(src) ?: return safeCrop(src)
        ...
        if (!m.setPolyToPoly(srcPts,0,dstPts,0,4)) return safeCrop(src)
        Canvas(dst).drawBitmap(src,m,Paint(Paint.ANTI_ALIAS_FLAG or Paint.FILTER_BITMAP_FLAG))
        return dst
    }

    private fun safeCrop(b: Bitmap): Bitmap {
        val margin=(min(b.width,b.height)*0.035f).roundToInt()
        val w=(b.width-margin*2).coerceAtLeast(1)
        val h=(b.height-margin*2).coerceAtLeast(1)
        return Bitmap.createBitmap(b,margin,margin,w,h)
    }
    ...
    private fun detectQuad(src: Bitmap): Quad? {
        ...
        for(x in 10 until w-10 step max(1,w/80)) {
            var bestT=0f; var bestY=1
            for(y in 5 until max(6,h/3)) {
            ...
            if(bestT>18) top+=PointF(x.toFloat(),bestY.toFloat())
            if(bestB>18) bottom+=PointF(x.toFloat(),by.toFloat())
        }
        val lf=mutableListOf<PointF>(); val rt=mutableListOf<PointF>()
        for(y in 10 until h-10 step max(1,h/80)) {
            var bestL=0f; var bx=1
            for(x in 5 until max(6,w/3)) {
            ...
            if(bestL>18) lf+=PointF(bx.toFloat(),y.toFloat())
            if(bestR>18) rt+=PointF(rx.toFloat(),y.toFloat())
        }
        val t=fit(top) ?: return null
        val bo=fit(bottom) ?: return null
        fun fitX(ps:List<PointF>):Pair<Float,Float>? {
            val swapped=ps.map{PointF(it.y,it.x)}; val f=fit(swapped) ?: return null
            return f
        }
        val l=fitX(lf) ?: return null
        val r=fitX(rt) ?: return null
        fun intersectY(a:Float,b:Float,c:Float,d:Float):PointF? {
            val den=1f-c*a
            if(abs(den)<0.02f)return null
            val x=(c*b+d)/den; val y=a*x+b
            return PointF(x,y)
        }
        val tl=intersectY(t.first,t.second,l.first,l.second) ?: return null
        val tr=intersectY(t.first,t.second,r.first,r.second) ?: return null
        val bl=intersectY(bo.first,bo.second,l.first,l.second) ?: return null
        val br=intersectY(bo.first,bo.second,r.first,r.second) ?: return null
        val pts=arrayOf(tl,tr,br,bl)
        val ok=pts.all{it.x in 0f..w.toFloat() && it.y in 0f..h.toFloat()}
        if(!ok)return null
        return Quad(pts.map{PointF(it.x/scale,it.y/scale)}.toTypedArray(),
            max(distance(tl,tr),distance(bl,br)).roundToInt(),
            max(distance(tl,bl),distance(tr,br)).roundToInt())
    }
